...
void LCD_DrawCircle(uint16_t usC, uint16_t usP, uint16_t R, uint16_t usColor)
{
	 ...
	 */
	uint16_t x, y, epsilon, circle;
	epsilon = 100;
	for (x = 0; x <= 320; x++)
	{
		for (y = 0; y <= 240; y++)
		{
			circle = (x - usC) * (x - usC) + (y - usP) * (y - usP);
			if ((circle <= R * R + epsilon) && (circle >= R * R - epsilon))
			{
...
void ILI9341_GramScan(uint8_t ucOption)
{
	...
	LCD_Write_Cmd(0x36);
	LCD_Write_Data(0x08 | (ucOption << 5)); //????ucOption???????LCD????????0-7??g?
	LCD_Write_Cmd(CMD_SetCoordinateX);
	LCD_Write_Data(0x00);							  /* x ??'????E?8? */
	LCD_Write_Data(0x00);							  /* x ??'????E?8? */
	LCD_Write_Data(((LCD_X_LENGTH - 1) >> 8) & 0xFF); /* x ????????E?8? */
	LCD_Write_Data((LCD_X_LENGTH - 1) & 0xFF);		  /* x ????????E?8? */

	LCD_Write_Cmd(CMD_SetCoordinateY);
	LCD_Write_Data(0x00);							  /* y ??'????E?8? */
	LCD_Write_Data(0x00);							  /* y ??'????E?8? */
	LCD_Write_Data(((LCD_Y_LENGTH - 1) >> 8) & 0xFF); /* y ????????E?8? */
	LCD_Write_Data((LCD_Y_LENGTH - 1) & 0xFF);		  /* y ????????E?8? */
	LCD_Write_Cmd(CMD_SetPixel);
}
